...
$(document).ready(function () {
  $.ajax({
    ...
    success: function (data) {
      ...
      let countSales=0;
      data.rentas.forEach(function (item) {
      ...
      if (
        !(
          data.publication.owner.image == null ||
          data.publication.owner.image == ""
        )
      ) {
        profile = serverURL + "image/" + data.publication.owner.image;
      }
    var imagenes='<div class="slider-container">'+
    '<div class="slider" id="slider">';
    data.publication.images.forEach(function (item) {
        imagenes+=' <div><img src="' + serverURL +"image/" +item+'" alt=""> </div>';
        
    });
    imagenes+='</div><a href="" class="switch" id="prev"><span></span></a><a href="" class="switch" id="next"><span></span></a></div>';
      ...
      var html =
        ...
        ');"></div>' +
        '<p class="t1">' +
        data.publication.owner.fullname +
        "</p>" +
        '<p class="t2">' +
        data.publication.locationId.locality +
        ", " +
        data.publication.locationId.state +
        "</p>" +
        '<p class="t2"> Rentas del usuario: ' +
        countSales +
        "</p>" +
        "</center>" +
        "</div>" +
        ...
        '<div class="col-md-12">' +
        "<br>" +
        '<a class="btn btn-calificar-renta" onclick="copiar_link()" role="button">Compartir enlace</a>' +
        "</div>" +
        "</div>" +
        "</div>" +
        "</div>" +
...
function rentar() {
  ...
  $.ajax({
    ...
    error: function (error) {
      ...
      if (
        error.responseJSON.errors.errors.daysReservation ==
        "daysReservation.must.to.be.after.today"
      ) {
        swal(
          "Error!",
          "Asegurese que la fecha seleccionada sea mayor al dia actual",
          "error"
        );
      }
    },
